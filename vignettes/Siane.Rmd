...
```{r}
df <- adf <- read.csv(system.file("extdata","sampleRiojaPopulation.csv",package = "Siane"))
...
```{r}
by <- "codes"
title_plot <- "Población total por municipios en Zaragoza"
value <- "value"
...
```{r}
df <- read.csv(system.file("extdata","sampleSpainPopulation.csv",package = "Siane"))
names(df) # List the column names  of the data frame
```
...
Reading the data
```{r}
ine_path <- read.csv(system.file("extdata","sampleBarcelonaAges.csv",package = "Siane"))
